...
# function to compare grades of 2 classes
...
get_letter_grade <- function(df, col_name) {
  for (i in 1:nrow(df)) {
    grade = df[i,col_name]
    if (grade == 1) {
      df[i,col_name] = "A"
...
compare_grades <- function(course1, course2) {
  ...
  ggplot(df2, aes(x=course1_grade, fill=grade)) +
    ...
    labs(title = paste("Performance in", as.character(course1), "vs", as.character(course2)), x = xlabel, y = ylabel)
}

#compare_grades(36315, 36226)
```
...
compare_grades_text <- function(course1, course2) {
  ...
  prop_table = prop.table(table(df2$course1_grade, df2$grade))
  
  hold = NULL
  for (i in 1:5) {
    percentage = paste(as.character(round(prop_table[i,i] * 100, digits = 2)), "%", sep = "")
    letter_grade = " "
    if (i == 1) {letter_grade = "A"}
    if (i == 2) {letter_grade = "B"}
    if (i == 3) {letter_grade = "C"}
    if (i == 4) {letter_grade = "D"}
    if (i == 5) {letter_grade = "R"}
    
    text = paste(percentage, "of students who get a", letter_grade, "in", as.character(course1), "also get a", letter_grade, "in", as.character(course2))
...
#compare_grades_text(36401, 36402) #bug
```
...
##Bugs: image rendering, Emily EDA
...
server = function(input,output){
  ...
  output$plot4 <- renderPlot({
    compare_grades(input$course_id_comparative_1, input$course_id_comparative_2)
  },  height = 400, width = 900)
