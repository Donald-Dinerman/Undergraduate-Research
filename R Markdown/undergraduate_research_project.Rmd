...
course_performance_by_major = function(course_n, table = F){
...
course_performance_by_major(course_n = 36226, table = F)
```
...
##need to build a dynamic ui that reacts to user input of course_outcome_1 (https://shiny.rstudio.com/articles/dynamic-ui.html): use conditional
    ...
    tabPanel("Course Performance by Major",
       ...
       fluidPage(
        ...
        mainPanel(
          ...
          column(width = 8, tableOutput("table2"))#issue with table 2 rendering outside of app (shiny.tag.list)
 )
    )
  ))
...
server = function(input,output){
  ...
  output$table2 <- renderText({course_performance_by_major(course_n = input$course_id_by_major, table = T)}) 
  #kbl and data tables rendering outside of app (issue on ui side) (shiny.tag.list)
  ...
  output$plot3 <- renderPlot({
    course_performance_by_major(course_n = input$course_id_by_major, table = F) #add table integration later
  })
