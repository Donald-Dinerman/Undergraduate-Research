...
*this needs work*(may not need grad year data/going to get this from Amanda)
...
grade_prob = c(0.4, 0.2, 0.2, 0.1, 0.1) #this information from model; model will have to be reactive to outcome course selection
...
grade_ev_func = function(letter = letter_grades, probabilities = grade_prob){
  ...
  grade_ev = sum(numeric_grades * probabilities)
...
course_options = prop_class_grade %>% filter(n_students > 100) %>% pull(1) %>% sort() %>% as.character()
...
body <- dashboardBody(
  ...
  sidebarPanel( 
    selectInput(inputId = "course_grade_1", label = "21256:", choices = letter_grades),
    selectInput(inputId = "course_grade_2", label = "36225:", choices = letter_grades),
    selectInput(inputId = "course_grade_3", label = "36226:", choices = letter_grades),
    selectInput(inputId = "course_grade_4", label = "36401:", choices = letter_grades),
...
top_four_retake = top_n_classes[1:4] #to get course number for retakes

ui = navbarPage(title = "Stat & DS Department",
  ...
  tabPanel("Predictive Modeling",
           ...
           fluidPage(
             ...
             sidebarPanel( 
                selectInput(inputId = "course_outcome_1", label = "Predict this course:", choices = top_four_retake), #top_four_retake is a placeholder
                selectInput(inputId = "course_grade_1", label = "21256:", choices = letter_grades),
                selectInput(inputId = "course_grade_2", label = "36225:", choices = letter_grades),
                selectInput(inputId = "course_grade_3", label = "36226:", choices = letter_grades),
                selectInput(inputId = "course_grade_4", label = "36401:", choices = letter_grades)),
  ...
  navbarMenu("Course EDA",
    ...
    fluidPage(
        ...
        titlePanel(title = h4("Course Enrollement Frequency", align = "center")),
  ...
  navbarMenu("Retake EDA",
    ...
    fluidPage(
        ...
        titlePanel(title = h4("Course Retake Frequency", align = "center")),
...
server = function(input,output){
  ...
  output$table2 <- renderText({top_n_retake_tab(n_retake = input$n_retake)})
