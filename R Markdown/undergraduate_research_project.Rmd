...
ui =  navbarPage(title = div(img(
  ...
  mainPanel(plotOutput("plot1"),
            ...
            dataTableOutput("data_table_123"),
...
```{r}
#server (navbar)
#Bug: Predict() Function is not outputting anything, issue with how the inputs are being entered/read
...
server = function(input,output){
  ...
  ##(3) Make this output some object and connect it to the predict button (create all the output behind the scenes and then just show it when the predict button is clicked)

  x_value = reactive(
    
    if(input$course_outcome == "15122"){
      
      c(input$class_id_1, input$major_1, input$units_carried_1, input$course_grade_1_1, input$course_grade_1_2,
    ...
    if(input$course_outcome == "15351"){
      ...
      c(input$class_id_2, input$major_2, input$units_carried_2, input$course_grade_2_1, input$course_grade_2_2, 
    ...
    if(input$course_outcome == "36225"){
      ...
      c(input$class_id_3, input$major_3, input$units_carried_3, input$course_grade_3_2, input$course_grade_3_1, input$course_grade_3_3, 
        input$cum_gpa_3, input$major_gpa_3)
              
      } else
                          
    if(input$course_outcome == "36226"){
        
      c(input$class_id_4, input$major_4, input$units_carried_4, input$course_grade_4_1, input$course_grade_4_2, input$course_grade_4_3,
        input$cum_gpa_4, input$major_gpa_4)
        
      } else
        
    if(input$course_outcome == "36401"){
      
      c(input$class_id_5, input$major_5, input$units_carried_5, input$course_grade_5_2, input$course_grade_5_3, input$course_grade_5_1,
        input$cum_gpa_5, input$major_gpa_5)
      
      } else
          
    if(input$course_outcome == "36402"){
          
      c(input$class_id_6, input$major_6, input$units_carried_6, input$course_grade_6_1, input$course_grade_6_2, input$course_grade_6_3,
        input$cum_gpa_6, input$major_gpa_6)
  ...
  #Pred Model Graph df
  #for when Predict() is working
  
  # graph_df = reactive({
  #   
  #   graph_table = data.frame(grade_char = c("A", "B", "C", "D"), grade_probabilities = model_ouput_shiny())
  #   
  #   graph_table
  # })
  ...
  output$data_table_123 <- renderDataTable({x_value_df()})
  ...
  #predict(input$course_outcome, x_value()) #put y_value in first argument
  
  output$text1234 <- eventReactive(input$go, {getFeatures(input$course_outcome)})
  ...
  #plots
  #Plot for when the predict() output is working
  # output$plot1 <- renderPlot({
  #   
  #   ggplot(graph_df(), aes(x = grade_char, y = grade_probabilities)) +
  #     geom_col(col = "Black", fill = "Steelblue") +
  #     scale_x_discrete(limits = c("D or R", "C", "B", "A" )) +
  #     coord_flip() +
  #     labs(title = "Predicted Grade Distribution", x = "Letter Grades", y = "Probability")
  #   
