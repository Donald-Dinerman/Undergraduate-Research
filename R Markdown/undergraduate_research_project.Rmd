...
compare_groups <- function(courseType, courseRange) {
  ...
  performance <- result %>%
    add_count(major) %>%
    add_count(major, grade) %>%
    ggplot(aes(x = major, y = nn / n, fill = grade)) +
           ...
           fluidPage(
             ...
             conditionalPanel(
               condition = "input.course_outcome_1 == '36402'",
                ...
                selectInput(inputId = "class_id", label = "Class:", choices = as.character(1:5)),
                selectInput(inputId = "major", label = "Major:", choices = major_options[-1]),
                ...
                numericInput(inputId = "units_carried", label = "Units Carried:", value = 0, min = 0, max = 100, step = 1),
               
                numericInput(inputId = "gpa", label = "Cumulative GPA:", value = 0, min = 0, max = 4, step = 0.01),
                numericInput(inputId = "gpa", label = "Major GPA:", value = 0, min = 0, max = 4, step = 0.01)
             ),
