...
get_unique_classes = function(df){
...
#stat
stat_core = get_unique_classes(stat_core_df)

stat_elective = get_unique_classes(stat_elective_df) 

#Econ/Stat
econ_stat_core = get_unique_classes(econ_stat_core_df)

econ_stat_elective_remove = c(73374, 73363, 73407, 73450) #cannot include vec
econ_stat_elective_pre_removal = get_unique_classes(econ_stat_elective_df) 
econ_stat_elective = econ_stat_elective_pre_removal[econ_stat_elective_pre_removal %!in% econ_stat_elective_remove]

#Stat/ML
stat_ml_core = get_unique_classes(stat_ml_core_df)

stat_ml_elective_remove = c(36410) #cannot include vec
stat_ml_elective_pre_removal = get_unique_classes(stat_ml_elective_df)
stat_ml_elective = stat_ml_elective_pre_removal[stat_ml_elective_pre_removal %!in% stat_ml_elective_remove]
```

*Note: The data Amanda gave us classifies any 363xx class as a core even though a course like 36-315 is a pure elective*
*Do we need to make a solid list of core (without the x's) and then %!in% it to the elective vec with the x's to avoid double counting (as core/elective)*
```{r}
...
```{r}
#Find times where course is a core and elective
#Small example, these courses are classified as both core and elective based on the excel file, how should we fix?
stat_df %>% 
  filter(stat_core == 1, stat_elective == 1) %>%
  select(course) %>%
  unique()
```
...
*this needs work*(may not need grad year data)
```{r}
...
```
```{r}
#Prove that the data includes students who didn't fully matriculate (less than senior)

proof_df = clean_joined_updated_QPA_df %>%
  group_by(student) %>%
  arrange(semester) %>%
  summarise_all(last)

table(proof_df$class_id)
```
...
Predict categorical distribution of grades (A, B, C, D, R) using pre-req data for a select number of core intermediate courses

Can use a gradient boosting model

```{r, include=F}
#example url of model
https://stackoverflow.com/questions/29454883/in-gbm-multinomial-dist-how-to-use-predict-to-get-categorical-output/29456904
```
