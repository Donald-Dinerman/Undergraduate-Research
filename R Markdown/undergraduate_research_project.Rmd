...
for(i in 1:5) {
  course_grade_func(top_n_classes[i], label = F)
}
...
capstone_courses = first_and_last_takes %>% filter(grade %in% c("P", "A")) %>%
  group_by(student, course) %>%
  summarise(count = n()) %>%
  filter(count == 2) %>%
  pull(2) %>%
...
for(i in 1:3) {
  retake_course_grade_func(top_n_classes[i], label = F)
}
