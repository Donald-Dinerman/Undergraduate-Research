...
```{r}
# get data frame that just has last takes of all classes
df = read.csv("wrangled_data.csv")

n_students = df$student %>% unique() %>% length()

course_counts = df %>%
  group_by(student, course) %>%
  summarise(times_taken = n())

complete = left_join(df, course_counts, by = c("student", "course"))

last_takes = complete %>%
  group_by(student, course) %>%
  slice_max(n = 1, order_by = semester)

last_takes = as.data.frame(subset(last_takes, select = c(student, semester, course, grade, times_taken)))

last_takes$grade = as.character(last_takes$grade)
last_takes$student = as.character(last_takes$student)
```
...
compare_grades_plot <- function(course1, course2) {
  ...
  df1 = get_course_df(course1, last_takes)
  df2 = get_course_df(course2, last_takes)
...
#compare_grades_plot(36225, 36226)
```
...
#compare_grades_text(36225, 36226)
```
...
Shiny App that can host the model and functional EDA

```{r}
#Python Conversion Script
## check predict.py and models/ folder exists
## source_python("predict.py") gives you two functions:
## getFeatures(course_number) and predict(course_number, df)
## note there are python module dependencies: pandas and other modules
## also need to install either miniconda or anaconda using a path with no spaces to use reticulate library (you will be prompted to do this in the console when running the source_python() function)

#Uncomment the code below (7 lines), this code creates a virtual environment with the necessary dependencies
#library(reticulate)
#version <- "3.8.7"
#install_python(version = version) #only need to run this once
#virtualenv_create("r-reticulate", python_version = version) #only need to run this once
#virtualenv_install("r-reticulate", packages=c("scikit-learn", 'pandas')) #only need to run this once
#use_virtualenv("r-reticulate")
#source_python("predict.py")

#Comment the code below (3 lines), I already have the dependencies installed so I don't need that commented code above
library(reticulate)
use_virtualenv("venv")
source_python("predict.py")
```
...
```{r}
#EV grade calculator
letter_grades = c("A", "B", "C", "D")

letter_grades_transfer = c(letter_grades, "T")

grade_ev_func = function(letter = letter_grades, probabilities){
...
top_four_outcome_pred = c(15122, 15351, 36225, 36226, 36401, 36402)
  ...
  src = "https://www.stat.cmu.edu/wids/logo/cmu-stat.png", height = 30, width = 150), ""),
      ...
      tabPanel("Predictive Modeling",
           ...
           fluidPage(
             ...
             sidebarPanel( 
                selectInput(inputId = "course_outcome", label = "Predict Course:", choices = top_four_outcome_pred), 
             
             conditionalPanel(
               condition = "input.course_outcome == '15122'",
                ...
                selectInput(inputId = "course_grade_1_1", label = "15112:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_1_2", label = "21127:", choices = letter_grades_transfer),
             ...
             conditionalPanel(
               condition = "input.course_outcome == '15351'",
                ...
                selectInput(inputId = "course_grade_2_1", label = "15122:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_2_2", label = "21127:", choices = letter_grades_transfer),
             ...
             conditionalPanel(
               condition = "input.course_outcome == '36225'",
                ...
                selectInput(inputId = "course_grade_3_1", label = "21120:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_3_2", label = "21241:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_3_3", label = "21256:", choices = letter_grades_transfer),
             ...
             conditionalPanel(
               condition = "input.course_outcome == '36226'",
                ...
                selectInput(inputId = "course_grade_4_1", label = "21241:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_4_2", label = "21256:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_4_3", label = "36225:", choices = letter_grades_transfer),
             ...
             conditionalPanel(
               condition = "input.course_outcome == '36401'",
                ...
                selectInput(inputId = "class_id_5", label = "Class:", choices = as.character(1:5)),
                selectInput(inputId = "major_5", label = "Major:", choices = major_options[-1]),
               
                selectInput(inputId = "course_grade_5_1", label = "21241:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_5_2", label = "36225:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_5_3", label = "36226:", choices = letter_grades_transfer),
                              
                numericInput(inputId = "units_carried_5", label = "Units Carried:", value = 0, min = 0, max = 100, step = 1),
               
                numericInput(inputId = "cum_gpa_5", label = "Cumulative GPA:", value = 0, min = 0, max = 4, step = 0.01),
                numericInput(inputId = "major_gpa_5", label = "Major GPA:", value = 0, min = 0, max = 4, step = 0.01)
             ),
             
             conditionalPanel(
               condition = "input.course_outcome == '36402'",
               
                selectInput(inputId = "class_id_6", label = "Class:", choices = as.character(1:5)),
                selectInput(inputId = "major_6", label = "Major:", choices = major_options[-1]),
               
                selectInput(inputId = "course_grade_6_1", label = "36225:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_6_2", label = "36226:", choices = letter_grades_transfer),
                selectInput(inputId = "course_grade_6_3", label = "36401:", choices = letter_grades_transfer),
                              
                numericInput(inputId = "units_carried_6", label = "Units Carried:", value = 0, min = 0, max = 100, step = 1),
               
                numericInput(inputId = "cum_gpa_6", label = "Cumulative GPA:", value = 0, min = 0, max = 4, step = 0.01),
                numericInput(inputId = "major_gpa_6", label = "Major GPA:", value = 0, min = 0, max = 4, step = 0.01)
             ),
             ...
             bsTooltip(id = "class_id_5", title = "Input Class (e.g., Freshman = 1, Sophmore = 2)"),
             bsTooltip(id = "class_id_6", title = "Input Class (e.g., Freshman = 1, Sophmore = 2)"),
             ...
             bsTooltip(id = "major_5", title = "Input Student\\'s Major"),
             bsTooltip(id = "major_6", title = "Input Student\\'s Major"),
             ...
             bsTooltip(id = "units_carried_5", title = "Input Units Carried During Semester of Predicted Course"),
             bsTooltip(id = "units_carried_6", title = "Input Units Carried During Semester of Predicted Course"),
             ...
             bsTooltip(id = "cum_gpa_5", title = "Input Cumulative GPA (Find on SIO)"),
             bsTooltip(id = "cum_gpa_6", title = "Input Cumulative GPA (Find on SIO)"),
             ...
             bsTooltip(id = "major_gpa_5", title = "Input Major GPA (Find on Stellic)"),
             bsTooltip(id = "major_gpa_6", title = "Input Major GPA (Find on Stellic)"),
             ...
             bsTooltip(id = "course_grade_5_1", title = "Input Course Grade"),
             bsTooltip(id = "course_grade_5_2", title = "Input Course Grade"),
             bsTooltip(id = "course_grade_5_3", title = "Input Course Grade"),
             
             bsTooltip(id = "course_grade_6_1", title = "Input Course Grade"),
             bsTooltip(id = "course_grade_6_2", title = "Input Course Grade"),
             bsTooltip(id = "course_grade_6_3", title = "Input Course Grade"),
...
server = function(input,output){
  ...
  x_value = reactive(
    ...
    if(input$course_outcome == "15122"){
      
      c(as.numeric(input$class_id_1), input$major_1, input$units_carried_1, input$course_grade_1_1, input$course_grade_1_2,
        input$cum_gpa_1, input$major_gpa_1)
      
      } else
          
    if(input$course_outcome == "15351"){
          
      c(as.numeric(input$class_id_2), input$major_2, input$units_carried_2, input$course_grade_2_1, input$course_grade_2_2, 
        input$cum_gpa_2, input$major_gpa_2)
          
      } else
                        
    if(input$course_outcome == "36225"){
                          
      c(as.numeric(input$class_id_3), input$major_3, input$units_carried_3, input$course_grade_3_2, input$course_grade_3_1,
        input$course_grade_3_3, input$cum_gpa_3, input$major_gpa_3)
              
      } else
                          
    if(input$course_outcome == "36226"){
        
      c(as.numeric(input$class_id_4), input$major_4, input$units_carried_4, input$course_grade_4_1, input$course_grade_4_2,
        input$course_grade_4_3, input$cum_gpa_4, input$major_gpa_4)
        
      } else
        
    if(input$course_outcome == "36401"){
      
      c(as.numeric(input$class_id_5), input$major_5, input$units_carried_5, input$course_grade_5_2, input$course_grade_5_3,
        input$course_grade_5_1, input$cum_gpa_5, input$major_gpa_5)
      
      } else
          
    if(input$course_outcome == "36402"){
          
      c(as.numeric(input$class_id_6), input$major_6, input$units_carried_6, input$course_grade_6_1, input$course_grade_6_2,
        input$course_grade_6_3, input$cum_gpa_6, input$major_gpa_6)
          
      }
  )
  
  #Make x_values readable for the predict function
  features = reactive(getFeatures(input$course_outcome)) 
  
  x_value_df <- reactive({ #create df and set colnames within one reactive element
    
    new_table = data.frame(matrix(nrow = 1, data = x_value()))
    colnames(new_table) = features()
    new_table
    
    })
  
  graph_df <- reactive({ #transform probability list into df for plot

   model_values = predict(input$course_outcome, x_value_df())

   y_var = model_values[[2]] %>% as.data.frame() %>% t()
   
   graph_table = data.frame(grade_char = c("A", "B", "C", "D"), grade_probabilities = y_var)
   
   graph_table

  })
  
  char_ev_grade_value <- reactive({ #get max prob letter grade list item
  ...
  num_ev_grade_value <- reactive({
    
    model_values = predict(input$course_outcome, x_value_df())

    grade_probs = model_values[[2]] %>% as.data.frame() %>% t() %>% as.vector()
    
    grade_ev_func(probabilities = grade_probs)
  })
  ...
  grade_reactive <- eventReactive(input$go, #put the expected grade text together
    {
      paste("Your expected grade is:", char_ev_grade_value(), "(", num_ev_grade_value(), ")") %>% 
        gsub("[[:space:]](?=[^()]*\\))", "", ., perl = T) #remove space within parentheses
      })
  
  output$text1 <- renderUI({grade_reactive()})
  ...
  predict_plot <- eventReactive(input$go, 
    
        {
          ggplot(graph_df(), aes(x = grade_char, y = grade_probabilities)) +
            geom_col(col = "Black", fill = "Steelblue") +
