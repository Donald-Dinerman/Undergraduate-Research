...
```{r}
#Prior major information (get from Alan rmd)

# major.changes.df <- df %>% group_by(student) %>% 
  # mutate(major_after = lead(major_)) %>% 
  # filter(major_ != major_after) %>% 
  # ungroup()

# major.changes.df %>% 
  # select(major_, major_after, everything())

# major.changes <- major.changes.df %>% 
  # select(major_, major_after)

# major.changes.counts <- major.changes %>%
#  count(., major_, major_after) %>% 
#   filter(major_ != "H00" & major_after != "H00") %>% 
#   arrange(desc(n))
# 
# before <- with(major.changes.counts, rep(x = major_, times = n))
# 
# after <- with(major.changes.counts, rep(x = major_after, times = n))
# 
# df2 <- data.frame(before, after)
```
