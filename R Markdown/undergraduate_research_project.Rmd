...
ui =  navbarPage(title = div(tags$img(src = "cmu-stat-ds.png", alt = "Error", deleteFile = F), "Stat and DS Advising"), #add image (bug: not locating image file)
      ...
      tabPanel("Predictive Modeling",
           ...
           fluidPage(
             ...
             sidebarPanel( 
                selectInput(inputId = "course_outcome", label = "Predict this course:", choices = top_four_outcome_pred), #choices x is a placeholder 
...
server = function(input,output){
  ...
  ##(1) I can make calls to the model function in this statement with the unique inputs within the model function.
  ##(2) Store the reactive output to take an EV grade and construct a ggplot bar chart of grade probabilities
  
  x_value = reactive(
    if(input$course_outcome == "36402"){input$major_1} else
                      if(input$course_outcome == "36401"){input$major_2} else
                        if(input$course_outcome == "73374"){input$major_3} else
                          if(input$course_outcome == "15351"){input$major_4}
  )
